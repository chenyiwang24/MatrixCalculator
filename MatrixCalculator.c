...
int main(void)
{
    int cont = 0;
    printWelcome();
    while (cont == 0) // Check to see if user wants to continue using
    {
        runMatrixCalc();
...
void runMatrixCalc(void)
{
    int selection = setSelection(); // Obtain the operation user wants to perform
    int correct = 0; // Ensures correct user inputs

    printf("\nEnter Left Side Matrix:");
    int leftRows = setDim(0), leftCols = setDim(1), rightRows, rightCols;
    if (selection > 0 && selection < 4) // Operations needing two matrices
    {
        printf("\nEnter Right Side Matrix:");
        rightRows = setDim(0), rightCols = setDim(1);
        // Add and subtract require matrices of same dimension
        if ((selection == 1 || selection == 2) &&
            (leftRows != rightRows || leftCols != rightCols))
        {
            correct++; // Mark as incorrect input
        }
        // Matrix multiplication require left matrix columns to equal
        // right matrix rows
...
{
    double scale; // Case for scalar, requires extra user input on scalar multiple
    if (selection == 4)
    {
    ...
    // Switch statement containing all operations
    switch (selection)
    {
    case 1:
...
int setSelection(void)
{
    ...
    int selection;
    scanf("%d", &selection);
    while (selection < 1 || selection > 5) // If selection invalid, redo
    {
...
int setDim(int dim)
{
    ...
    scanf("%d", &result);
    while (result < 1 || result > 10) // If invalid, redo
    {
...
void setMatrix(double matrix[MAX_SIZE][MAX_SIZE], int rows, int cols)
{
    ...
    for (int i = 0; i < rows; i++)
    {
        for (int j = 0; j < cols; j++)
        {
            ...
            scanf("%lf", &matrix[i][j]); // Sets the values of the matrix
        }
    }
}
...
void multiply(double left[][MAX_SIZE], double right[][MAX_SIZE], int rows, int cols)
{
    ...
    for (int i = 0; i < rows; i++)
    {
        for (int j = 0; j < cols; j++)
        {
            // Multiply and sums the left side rows times the right side columns
